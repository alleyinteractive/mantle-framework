...
class ArrTest extends TestCase {
	...
	public function testCrossJoin() {
		...
		$this->assertSame(
			[ [ 1, 'a' ], [ 1, 'b' ], [ 1, 'c' ] ],
			Arr::cross_join( [ 1 ], [ 'a', 'b', 'c' ] )
		);

		// Square matrix
		$this->assertSame(
			[ [ 1, 'a' ], [ 1, 'b' ], [ 2, 'a' ], [ 2, 'b' ] ],
			Arr::cross_join( [ 1, 2 ], [ 'a', 'b' ] )
		);

		// Rectangular matrix
		$this->assertSame(
			[ [ 1, 'a' ], [ 1, 'b' ], [ 1, 'c' ], [ 2, 'a' ], [ 2, 'b' ], [ 2, 'c' ] ],
			Arr::cross_join( [ 1, 2 ], [ 'a', 'b', 'c' ] )
		);
		...
		$this->assertSame(
			...
			Arr::cross_join( [ 1, 2 ], [ 'a', 'b' ], [ 'I', 'II', 'III' ] )
		);

		// With 1 empty dimension
		$this->assertEmpty( Arr::cross_join( [], [ 'a', 'b' ], [ 'I', 'II', 'III' ] ) );
		$this->assertEmpty( Arr::cross_join( [ 1, 2 ], [], [ 'I', 'II', 'III' ] ) );
		$this->assertEmpty( Arr::cross_join( [ 1, 2 ], [ 'a', 'b' ], [] ) );

		// With empty arrays
		$this->assertEmpty( Arr::cross_join( [], [], [] ) );
		$this->assertEmpty( Arr::cross_join( [], [] ) );
		$this->assertEmpty( Arr::cross_join( [] ) );

		// Not really a proper usage, still, test for preserving BC
		$this->assertSame( [ [] ], Arr::cross_join() );
	}
	...
	public function testIsAssoc() {
		...
		$this->assertFalse( Arr::is_assoc( [ 'a', 'b' ] ) );
	}
	...
	public function testSortRecursive() {
		...
		$this->assertEquals( $expect, Arr::sort_recursive( $array ) );
	}
