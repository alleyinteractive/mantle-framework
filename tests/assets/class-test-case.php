...
abstract class Test_Case extends Framework_Test_Case {
	protected function setUp(): void {
		...
		$this->app->singleton(
			'asset.map',
			fn() => Asset_Map::create( __DIR__ . '/../fixtures/assets/assetMap.json', 'default' ),
		);
		$this->app->alias( 'asset.map', Asset_Map::class );
	}

	protected function assertHeadContains( string $needle ) {
		$this->assertStringContainsString( $needle, $this->get_wp_head() );
	}
