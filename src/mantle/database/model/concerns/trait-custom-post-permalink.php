	...
	public static function boot_custom_post_permalink(): void {
		if ( static::get_route() ) {
			if ( 'post' === static::get_object_name() ) {
				add_filter( 'post_link', self::filter_post_type_link( ... ) );
			} else {
				add_filter( 'post_type_link', self::filter_post_type_link( ... ) );
			}
		}

		if ( static::get_archive_route() ) {
			add_filter( 'post_type_archive_link', self::filter_post_type_archive_link( ... ) );
		}
